...
mkdir $1/output
for file in $directoryname/*.pcap.enc.clr;
do
...
python iptoname.py $1/ipts/ip_ts_$name.txt $1/ip_to_identity.txt 
# Sorts the file by timestamps
cat namestotimestamps.txt | sort -k3,3 -t '"' -s >> $1/namestotimestamps/sorted_namestotimestamps.txt

# Splits each timestamps into separate days
python split.py $1/namestotimestamps/sorted_namestotimestamps.txt
